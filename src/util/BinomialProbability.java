...
public interface BinomialProbability {
	 ...
	 * @return La probabilité que l'événement se produise au moins " + N + " fois
	 *         après " + X + " tentatives
	 */
	...
	public static double binomialProbability(int tentativeNumber, int k, double succesProbability) {
		...
		return binomialCoefficient(tentativeNumber, k) * Math.pow(succesProbability, k)
				* Math.pow(q, tentativeNumber - k);
	}
	...
	public static int calculateNbTentative(double minProbabilityExpected, int succesNumber, double succesProbability,
			int maxTentative) {
		int tentativeNumber = succesNumber - 1;
		double currentProbability = 0d;
		while (minProbabilityExpected >= currentProbability && tentativeNumber < maxTentative) {
			tentativeNumber++;
			double probability = 0.0;
			for (int k = succesNumber; k <= tentativeNumber; k++) {
				probability += binomialProbability(tentativeNumber, k, succesProbability);
