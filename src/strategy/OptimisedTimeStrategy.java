...
public final class OptimisedTimeStrategy implements Strategy {
	...
	private void optimize(ArtifactGoalAchiever goalAchiever) {
		if (timeGoalAchieverMap.get(goalFactory.getInfos(goalAchiever).getItemCode())
			...
			goalFactory.getGoalAverageOptimizer().optimize(goalAchiever, Integer.MAX_VALUE, 0.9f);
		} else {
	...
	static List<Predicate<ArtifactGoalAchiever>> createFiltersPredicate(GoalFactory goalFactory, Bornes bornes) {
		...
		filterPredicate.add(StrategySkillUtils.createFilterCraftPredicate(goalFactory, BotCraftSkill.COOKING, bornes));
		filterPredicate.add(StrategySkillUtils.createFilterResourceAndCraftPredicate(goalFactory, bornes,
		...
		filterPredicate
				.add(StrategySkillUtils.createFilterResourcePredicate(goalFactory, bornes, BotResourceSkill.FISHING));
